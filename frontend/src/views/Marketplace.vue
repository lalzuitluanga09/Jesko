    ...
    <div>
        ...
        <div v-else>
            <div class="flex items-center justify-between my-2 mx-2 md:mx-0 md:my-4">
                <p v-if="filter.searchTerm" class="text-xs md:text-sm text-gray-600 dark:text-gray-400">Search result for : <span class="font-semibold">{{ filter.searchTerm }}</span></p>
                <p class="text-gray-600 dark:text-gray-400 text-xs md:text-sm ml-2 md:ml-0">Showing {{ pagination.from }}-{{ pagination.to }} of {{ pagination.total }} results</p>
                <!-- <SelectBtn /> -->
            </div>
            <div v-if="pagination.total > 0" class="flex-1 grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-2 md:gap-4 mx-1 md:mx-0">
...
import MarketSearchBar from '@/components/marketplace/MarketSearchBar.vue';
