  ...
  <div class="flex flex-col md:flex-row gap-4">
    <div class="flex-1">
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1" for="price"
        >Category</label
      >
        <VueSelect label="name" :reduce="(item: Category) => item.id" :options="allCategories" v-model="formData.category_ids" multiple/>
    </div>
    <div class="flex-1">
      <label class="block text-sm font-medium text-gray-700 dark:text-gray-200 mb-1" for="stock"
        >Tag</label
      >
        <VueSelect label="name" :reduce="(item: Tag) => item.id"  :options="allTags" v-model="formData.tag_ids" multiple/>
    </div>
  </div>
