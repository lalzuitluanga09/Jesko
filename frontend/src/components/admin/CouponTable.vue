<template>
        ...
        <table class="w-full border border-gray-300 text-sm text-left rtl:text-right text-gray-500 dark:text-gray-400">
            <thead class="text-xs border-y border-gray-300 text-gray-700 uppercase bg-gray-50 dark:bg-gray-700 dark:text-gray-400">
                <tr>
                    ...
                    <th scope="col" class="px-6 py-3">Per User</th>
            ...
            <tbody>
                ...
                <tr v-else v-for="row in coupons" :key="row.id"
                    ...
                    <td class="px-6 py-4 cursor-pointer" @click="openViewDialog(row)">{{ row.code }}</td>
                    <td class="px-6 py-4 cursor-pointer" @click="openViewDialog(row)">
                        <span v-if="row.discountType === 'percentage'">
                            {{ Math.ceil(row.discountValue) }}%
                        </span>
                    ...
                    <td class="px-6 py-4 cursor-pointer" @click="openViewDialog(row)">{{ row.perUserLimit }}</td>
                    <td class="px-6 py-4 cursor-pointer" @click="openViewDialog(row)">
                        {{ formatToDatetime(row.endAt) }}
                    </td>
                    <td class="px-6 py-4 cursor-pointer" @click="openViewDialog(row)">
                        <span
    ...
    <ConfirmDeleteDialog :is-open="isOpen" @update:is-open="isOpen = $event" @deleteItem="deleteCoupon"/>
</template>
