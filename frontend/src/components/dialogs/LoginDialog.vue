      ...
      <div class="bg-white/80 dark:bg-gray-700/80 rounded-lg shadow-lg p-6 w-full max-w-sm">
        ...
        <form @submit.prevent="auth.isLogin ? auth.login() : auth.register()">
          ...
          <div class="mb-4">
            ...
            <input v-model="auth.formData.password" type="password" class="w-full border hover:bg-pink-50 dark:hover:bg-gray-600 border-gray-500 rounded px-3 py-2" required />
          </div>
  
          <div class="mb-4" v-if="!auth.isLogin">
            <label class="block text-gray-700 dark:text-gray-400 text-sm mb-2">Confirm Password</label>
            <input v-model="auth.formData.password_confirmation" type="password" class="w-full border hover:bg-pink-50 dark:hover:bg-gray-600 border-gray-500 rounded px-3 py-2" required />
          </div>
